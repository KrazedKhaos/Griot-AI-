...
const SearchPage: React.FC<SearchPageProps> = ({ products, onProductClick, wishlistedProductIds, onWishlistToggle, initialSearchTerm }) => {
        ...
        <>
            <div className="p-4">
                <div className="relative mb-4 flex justify-center items-center">
                    <button onClick={() => window.history.back()} className="absolute left-0 p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700" aria-label={t.goBack} title={t.goBack}>
                ...
                <div className="mt-4">
                    <div className="flex overflow-x-auto space-x-3 pb-2 scrollbar-hide">
                        <button
                            ...
                            className={`flex items-center px-4 py-2 text-sm font-semibold rounded-full whitespace-nowrap transition-colors ${appliedFilters.categories.length === 0 ? 'bg-uc-primary text-white' : 'bg-gray-200 text-uc-text dark:bg-gray-700 dark:text-gray-200'}`}
                        >
                        ...
                        {CATEGORIES.map(cat => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center px-4 py-2 text-sm font-semibold rounded-full whitespace-nowrap transition-colors ${isSelected ? 'bg-uc-primary text-white' : 'bg-gray-200 text-uc-text dark:bg-gray-700 dark:text-gray-200'}`}
                                >
                ...
                {showResults && (
                    <div className="flex justify-between items-center mt-6">
                        <p className="text-sm text-gray-600 dark:text-gray-300 font-medium">
                            {filteredProducts.length} {filteredProducts.length === 1 ? t.result : t.results} {t.found}
                        </p>
                ...
                <div className={showResults ? "mt-4" : "mt-6"}>
                    ...
                    {showResults && filteredProducts.length === 0 && (
                         <EmptyState title={t.noResultsFound} message={t.noResultsMessage} />
                    )}
                    {!showResults && (
                        <EmptyState title={t.searchEmptyTitle} message={t.searchEmptyMessage} />
                    )}
                </div>
            </div>
            <FilterModal 
