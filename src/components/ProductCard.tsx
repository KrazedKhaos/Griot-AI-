            ...
            >
                <div className="relative w-32 h-32 flex-shrink-0">
                    {firstMedia && firstMedia.type === 'image' && (
                        <img loading="lazy" className="absolute inset-0 w-full h-full object-cover" src={firstMedia.url} alt={product.title} />
                    )}
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden hover:shadow-lg dark:hover:shadow-gray-700/50 transition-shadow duration-300">
            ...
            <div className="p-4">
                ...
                <h3 onClick={() => onProductClick(product)} className="block mt-1 text-lg leading-tight font-semibold text-uc-secondary dark:text-gray-100 hover:underline cursor-pointer h-14" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                    {product.title}
                </h3>
                ...
                <p className="mt-2 text-uc-text dark:text-gray-300 h-12" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{product.benefitStatement}</p>
                <div className="mt-4 grid grid-cols-2 gap-2">
                    <button onClick={() => onProductClick(product)} className="w-full bg-gray-200 dark:bg-gray-700 text-uc-secondary dark:text-gray-200 font-bold py-2 px-4 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
                        {t.learnMore}
                    </button>
                    <a
                        ...
                        className="flex items-center justify-center w-full bg-uc-primary text-white font-bold py-2 px-4 rounded-lg hover:bg-opacity-80 transition-transform transform hover:scale-105"
                    >
                        {t.buyNow}
                    </a>
                </div>
